...
function removeAccount(root, name) {
  ...
  writeRegistry(root, reg)
  // On macOS the account's login lives in the Keychain, not the folder, so
  // deleting the folder alone would leave a live refresh token behind.
  try { require('./credstore').remove(configDir(root, name)) } catch { /* best effort */ }
...
/** True when this account has a login (i.e. has been signed in). */
function isAuthed(root, name) {
  try { return require('./credstore').exists(configDir(root, name)) } catch { return false }
}
