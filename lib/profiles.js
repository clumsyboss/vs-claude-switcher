...
function writeProfile(root, name, credentials, identity, fallback) {
  ...
  writeAtomic(blobPath(root, name), dpapi.protect(Buffer.from(JSON.stringify(payload), 'utf8')))
...
function importFromDir(root, name, dir, opts) {
  ...
  if (opts && opts.move) {
    const back = JSON.parse(dpapi.unprotect(fs.readFileSync(blobPath(root, name))).toString('utf8'))
...
function apply(root, name, opts) {
  ...
  const payload = JSON.parse(dpapi.unprotect(blob).toString('utf8'))
